...
class Test_Controller extends Template_Controller {
	...
	public function start($idtest="",$code="",$type="promotion"){
		 ...
		 if($insert && !empty($code)){
			 ...
			 {
				 $inserts =  $this->db->insert('payment', array('member_uid'=>$this->sess_cus['id'],'test_uid' => $idtest,'payment_date' => strtotime(date('m/d/Y')),'valid_until'=>1,'promotion_code'=>$code,'payment_price'=>isset($test['price'])?$test['price']:'0','daytest'=>isset($test['date'])?$test['date']:'0'));
			 }else{
			  	 $inserts =  $this->db->insert('payment', array('member_uid'=>$this->sess_cus['id'],'test_uid' => $idtest,'payment_date' => strtotime(date('m/d/Y')),'valid_until'=>1,'transaction_code'=>isset($test['price'])&&$test['price']>0?$code:'','payment_price'=>isset($test['price'])?$test['price']:'0','daytest'=>isset($test['date'])?$test['date']:'0'));
			 }
			 if(!empty($inserts))
			 {
