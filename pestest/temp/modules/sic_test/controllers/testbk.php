...
class Test_Controller extends Template_Controller {
	...
	public function showlist(){
		...
		foreach($mr['mlist'] as $key => $value){
				...
				$this->db->limit(1);
				$payment = $this->payment_model->get();
				if(isset($payment[0]['payment_date']))
	...
	public function start($idtest,$code=""){
		 ...
		 $insert = true;
		 foreach($payment as $value){
				 if((isset($test['date'])?strtotime("-".$test['date'] ." day"):strtotime('now')) <= $value['payment_date']){
						$insert = false;
				 }
		 }
		 if($insert){
			 $insert =  $this->db->insert('payment', array('member_uid'=>$this->sess_cus['id'],'test_uid' => $idtest,'payment_date' => strtotime(date('m/d/Y')),'valid_until'=>1,'promotion_code'=>$code));
			 if(!empty($insert))
			 {
	...
	public function testing()
	{
		...
		if(!empty($mist_cate)){
			...
			{ 
				...
				{
					$mist_cate[$j]['questionnaires'][$i]['answer'] = $this->answer_model->get_questionnaires($mlist[$i]['uid']);
				}
				...
				{
					$mist_cate[0]['questionnaires'][$i]['answer'] = $this->answer_model->get_questionnaires($mlist[$i]['uid']);
				}
		}
		$mr['typetest'] = 1;	
		...
		{ 	
				...
				foreach($question as $value){
					...
					if(isset($val['uid']) && $val['uid'] == $value['category_uid']){
						...
						{
							$mist_cate[$key]['questionnaires'][$i]['answer'] = $this->answer_model->get_questionnaires($mist_cate[$key]['questionnaires'][$i]['uid']);
				        ...
				        if(!empty($value)){
							$mist_cate[0]['questionnaires'][] = $value;
								...
								for($i=0;$i<count($mist_cate[0]['questionnaires']);$i++)
								{
									...
									$this->answer_model->get_questionnaires($mist_cate[0]['questionnaires'][$i]['uid']);
								}
							$qty_question++; 
							}
					}
		...
		if(isset($this->sess_cus['id'])){
			 ...
			 if(!empty($member) && strlen(trim($member['company_contact_email'])) > 0){
				...
				try {
					...
					$mail->Port = 465; 
					$mail->SMTPDebug  = 0; 
					//$mail->Username = 'haibh90@gmail.com'; // SMTP username
					//$mail->Password = 'khocviyeu'; // SMTP password    
					$gmail = array('result4.pestest@gmail.com','result.pestest@gmail.com');
					$mail->Username = $gmail[array_rand($gmail)];//'result1.pestest@gmail.com'; // SMTP username
					$mail->Password = 'pestest123'; // SMTP password    
	...
	private function send_email_outlook($record)
    {
	    ...
	    try {
			...
			//$mail->Password = 'khocviyeu'; // SMTP password    
			 $gmail = array('result4.pestest@gmail.com','result.pestest@gmail.com');
					$mail->Username = $gmail[array_rand($gmail)];//'result1.pestest@gmail.com'; // SMTP username
			//$mail->Username = 'result.pestest@gmail.com'; // SMTP username
			$mail->Password = 'pestest123'; // SMTP password    
