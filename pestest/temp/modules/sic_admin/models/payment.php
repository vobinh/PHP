...
class Payment_Model extends Model {
	...
	public function getpayment($id='')
	{
		$table_test = 'test';
		$table_member = 'member';
        $this->db->select($this->table_name.'.*',$table_test.'.uid as test_id' ,$this->table_name.'.payment_price as price',$table_test.'.test_title',$table_test.'.date',$table_member.'.uid as member_id',$table_member.'.member_fname',$table_member.'.member_lname');
        if($id != '')
        $this->db->{is_array($id)?'in':'where'}($this->table_name.'.'.$this->primary_key, $id);
		
        $this->db->join($table_test,'test.uid',$this->foreign_test);
