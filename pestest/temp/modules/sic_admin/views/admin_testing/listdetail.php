...
<tr class="row<?php if($id%2 == 0) echo 2 ?>" id="row_<?php echo isset($list['uid'])?$list['uid']:''?>">
    ...
    <img src="<?php echo url::base()?>uploads/answer/<?php echo $list['answer']['images']?>" />
    <?php }else { ?>
	<?php echo isset($list['answer']['answer'])?$list['answer']['answer']:''?>
    <?php } ?>
    </td>
    <td align="center">
	 <?php  if(isset($list['answerright'][0]['images']) && !empty($list['answer']['images']))
				 			    echo '<img src="'.url::base().
								'uploads/answer/'.$list['answerright'][0]['images'].'" width="20px" />';
						else    if(isset($list['answerright'][0]['answer']))
...
<form id="frm_display" name="frm_display" method="post" action="<?php echo $this->site['base_url']?>admin_testing/showlistdetail/">
