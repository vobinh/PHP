			  ...
			  <?php foreach($ans as $value){?>
                  <tr id="answerno1">
                    ...
                    <?php if(isset($value['images']) && !empty($value['images'])) {?>
                       <td style="background: none;"><img src="<?php echo url::base()?>uploads/answer/<?php echo $value['images']?>" />
                     ...
                     <?php } ?>
                    <td style="background: none;"><input name="type_ans[]" id="type_ans[]" type="checkbox"  <?php echo ($value['type']==1)?'checked="checked"':'';?> onclick="$(':checkbox').attr('checked',false);$(this).attr('checked',true)"/>
