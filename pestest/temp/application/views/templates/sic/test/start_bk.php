    ...
    <table cellspacing="5" cellpadding="5" class="testheader" width="100%" align="center" style="margin-bottom: 15px;">
        ...
        <tr>
        	<td valign="middle" colspan="3" class="description" align="left"><h3>Category</h3>
             <?php for($i=0;$i<count($mlist_cate);$i++) {?>
               <?php 
			    
				 $this->db->where('category.uid',$mlist_cate[$i]['parent_id']);
		         $mr_parent = $category_model->get();
				echo isset($mr_parent[0]['category']) && $parent_category != $mr_parent[0]['category']?'<div style="font-size:20px;padding-left:10px;">'.$mr_parent[0]['category'].' </div>':"";
				if(isset($mr_parent[0]['category']))
				$parent_category = $mr_parent[0]['category'];
				 ?>
              <div style="padding:20px;float:left;"><?php echo $mlist_cate[$i]['category'] ?> : <?php echo isset($mlist_cate[$i]['category_percentage'])?$mlist_cate[$i]['category_percentage'].'%':"" ?></div>
             <?php } ?>
