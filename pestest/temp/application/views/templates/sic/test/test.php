...
margin-bottom: -10px;font-size: 38px;color: rgb(255, 255, 255);display: block;right: 10%;width: 150px;font-weight: bold;height: 100px;text-align: center;" >
    ...
    <div class="ans">
        <?php for($j=0;$j<count($mlist[$k]['questionnaires'][$i]['answer']);$j++) {?>
        	...
        	<div style="float:left; line-height:1.5em;width:30px;">
              <?php if(isset($version) && $version ==8) {?>
               <input onClick="counteranswer()" id="radio<?php echo isset($mlist[$k]['questionnaires'][$i]['answer'][$j]['uid'])?$mlist[$k]['questionnaires'][$i]['answer'][$j]['uid']:"" ?>"  type="radio"
                 ...
                 name="radio<?php echo $mlist[$k]['questionnaires'][$i]['uid'] ?>" style="margin-top:0px;width:auto;" previousvalue="false">
              <?php }else{ ?>
               <label><input onClick="counteranswer()" id="radio<?php echo isset($mlist[$k]['questionnaires'][$i]['answer'][$j]['uid'])?$mlist[$k]['questionnaires'][$i]['answer'][$j]['uid']:"" ?>" class="radio4" type="radio"
                
                 value="<?php echo $mlist[$k]['questionnaires'][$i]['answer'][$j]['type'] ?>|<?php echo isset($mlist[$k]['questionnaires'][$i]['answer'][$j]['uid'])?$mlist[$k]['questionnaires'][$i]['answer'][$j]['uid']:"" ?>|<?php echo $mlist[$k]['questionnaires'][$i]['category_uid']?>"
                 name="radio<?php echo $mlist[$k]['questionnaires'][$i]['uid'] ?>" style="margin-top:7px;width:auto;" previousvalue="false">
                 
               <span class="radio4"></span></label>
              <?php } ?>
            ...
            <img src="<?php echo url::base()?>uploads/answer/<?php echo $mlist[$k]['questionnaires'][$i]['answer'][$j]['images']?>" />
            <?php }else { ?>
 ...
 <?php } ?>
   ...
   <?php if(isset($n)&& $n >0) {?>
    <button type="submit" name="btn_submit" id="btn_submit" class="button" style="<?php if($mr['qty_question']>10) echo 'display:none';?>" onclick="$(this).hide('slow');$('#loading').show('fast');$('#loading_icon').show('fast')"><span>Finish</span></button>
