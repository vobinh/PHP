...
<script type="text/javascript" src="<?php echo url::base()?>js/AmazonS3FileManager/filemanager.js"></script>
<script type="text/javascript">
var ckeditor = CKEDITOR.replace('erea_question');
AmazonS3FileManager.init({
  returnTo: 'ckeditor',
  editor: ckeditor,
  main_url: '<?php echo url::base()?>',
});

var ckeditor_1 = CKEDITOR.replace('erea_note');
AmazonS3FileManager.init({
  returnTo: 'ckeditor',
  editor: ckeditor_1,
  main_url: '<?php echo url::base()?>',
	...
	$.msgbox('Do you really want to delete?', {
		...
		function(re) {
			if(re == 'Yes') {
				...
				$.getJSON("<?php echo url::base()?>admin_questionnaires/deleteAnswer/"+ id,
					function(obj) {
						if(obj.status) {
