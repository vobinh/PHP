...
class Admin_questionnaires_Controller extends Template_Controller
{
	...
	public function AWS_upload(){
        ...
        if (! upload::valid($image) OR ! upload::type($image, array('jpg', 'jpeg', 'png', 'gif'))){
			...
			$actual_image_name = 'qt_'.md5($image['name'].time()).".".$image_extension;
            try {
                // Upload data.
                $result = $s3Client->putObject(array(
                    'Bucket'      => $s3_bucket,
                    'Key'         => 'question_img/'.$actual_image_name,
                    'SourceFile'  => $tmp,
                    'ACL'         => 'public-read',
            ...
            $arr_out = array(
				'error' => 'Make sure image file is valid!',
				'name'  => '',
			);
			echo json_encode($arr_out);
			die();
	...
	public function AWS_Manager($type=''){
		require_once Kohana::find_file('views/aws','init');
		if($type == 'ajax')
			$template = new View('admin_questionnaires/AWS_item');
		else
			$template = new View('admin_questionnaires/AWS_manager');
		...
		//die();
		$template->set(array(
			'list_img' => !empty($iterator)?$iterator:'',
    ...
    public function s3_resize($file_name, $max_width, $max_height){
    ...
    protected function _save_img_s3($image){
        ...
        if (! upload::valid($image) OR ! upload::type($image, array('jpg', 'jpeg', 'png', 'gif'))){
            ...
            try {
                ...
                $result = $s3Client->putObject(array(
                    'Bucket'      => $s3_bucket,
                    'Key'         => 'answer/'.$actual_image_name,
                    'SourceFile'  => $this->s3_resize($tmp,593,539),
                    'ACL'         => 'public-read',
                    'ContentType' => 'image/png'
                ));
                return $actual_image_name;
			...
			|| (isset($array_id[$i]))){
				...
				if(!isset($array_id[$i])){
					...
					if(isset($_FILES['answer_file'.$i]) && !empty($_FILES['answer_file'.$i]['name'])){
						...
						// }
						// else 
						// 	$file_images = $this->upload_file($file['name'],$file);

						$file_images = $this->_save_img_s3($file);
						if($file_images == false)
							$file_images = '';
